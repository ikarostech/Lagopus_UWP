...
namespace Lagopus_UWP
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        public Dictionary<string, OAuth> AccountDictionary = new Dictionary<string, OAuth>();
        ...
        protected override async void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            if (newcomer != null)
            {
                if (newcomer[3] != "")
                {
                    ...
                    NewAccount.AccessKey = newcomer[0];
                    NewAccount.AccessSecret = newcomer[1];
                    NewAccount.ScreenName = newcomer[2];
                    NewAccount.UserId = newcomer[3];
                    var user = new User();
        ...
        //TLリストビューにアカウントを追加します
        public void Add_TimeLineListView(Tweet tweet, OAuth user)
        {
            //いろんなところで使うidを確保
        ...
        //リストボックス1用の更新ボタンです
        private async void Refresh1_Click(object sender, RoutedEventArgs e)
        {
            //いったんリフレッシュする
            ListView1.Items.Clear();
        }
        ...
        //ストリーミングに接続
        public async Task StreamGet(OAuth oauth, string APIURL, SortedDictionary<string, string> postdata)
        {
            SortedDictionary<string, string> Header = OAuthUtility.HeaderCreate(APIURL, oauth, HttpMethod.Get, postdata);
        ...
        //ストリーミングホームライムラインの取得
        public async Task Shome_timeline(OAuth user)
        {
            string APIURL = "https://userstream.twitter.com/1.1/user.json";
            SortedDictionary<string, string> postdata = new SortedDictionary<string, string>();
            await StreamGet(user, APIURL, postdata);
        }

        //にゃーんとつぶやいた回数を記録します
        public Dictionary<string, int> Nyan = new Dictionary<string, int>();
    ...
    public static class Statuses
    {
        ...
        public async static Task<Tweet> update(OAuth oauth, string status)
        {
            ...
            string resultstr = await HttpRequest.RestPOST(oauth, APIURL, postdata);
        ...
        public async static Task<Tweet> show(OAuth oauth, string id)
        {
            ...
            string resultstr = await HttpRequest.RestGET(oauth, APIURL, postdata);
            Tweet result = Serialize.JsontoTweet(resultstr);
            return result;
        }
        ...
        //レストホームタイムラインの取得
        public async static Task<List<Tweet>> Rhome_timeline(OAuth user)
        {
            string APIURL = "https://api.twitter.com/1.1/statuses/home_timeline.json";
            SortedDictionary<string, string> postdata = new SortedDictionary<string, string>();
            string resultstr = await HttpRequest.RestGET(user, APIURL, postdata);
            List<Tweet> result = Serialize.JsontoTweets(resultstr, user);
            return result;
        }
    ...
    public static class Users
    {
        ...
        public async static Task<User> show(OAuth oauth)
        {
            ...
            string result = await HttpRequest.RestGET(oauth, APIURL, postdata);
    ...
    public class OAuth
    {
        public string ConsumerKey { get { return ""; } }
        public string ConsumerSecret { get { return ""; } }
